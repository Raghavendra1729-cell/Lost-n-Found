...
const FoundList = ({ items, onMatches, onArchive, onDelete, matches, currentUser, onResolve, onOpenEnhancedChat }) => {
      ...
      <div className="space-y-4">
          ...
          <div key={it._id} className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-700/50 professional-bg">
            ...
            {matches[it._id] && (
              <div className="mt-4 border-t border-gray-600/50 pt-4">
                <div className="text-sm font-medium text-gray-300 mb-3 professional-text-shadow">Potential Owners ({matches[it._id].length}):</div>
                  ...
                  {matches[it._id].map((m) => (
                    <div key={m._id} className="flex gap-3 p-3 bg-gray-800/50 rounded-lg border border-gray-600/30">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-400 mt-1">Lost by: {m.userId?.name || 'Unknown'}</div>
                      </div>
                      <button 
                        onClick={() => handleContactClick(m)}
                        className="px-3 py-1 text-xs bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-300"
                      >
                        Contact Owner
                      </button>
                    </div>
        ...
        {items.length === 0 && (
          <div className="text-center py-8">
            <div className="text-gray-500 text-lg">No found items</div>
            <div className="text-gray-600 text-sm mt-2">Report a found item to help others</div>
          </div>
        )}
      </div>

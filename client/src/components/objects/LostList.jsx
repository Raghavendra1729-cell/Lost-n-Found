...
const LostList = ({ items, onMatches, onArchive, onDelete, matches }) => {
  const lost = items.filter((it) => it.type === 'lost' && it.status !== 'resolved')
  return (
      ...
      {lost.map((it) => (
        <div key={it._id} className="border border-gray-800 rounded p-3">
          ...
          <div className="mt-2 flex gap-2">
            <button onClick={() => onMatches(it._id)} className="px-3 py-1 rounded bg-gray-700 hover:bg-gray-600">Check matches</button>
          ...
          {matches[it._id] && (
            <div className="mt-2 border-t border-gray-800 pt-2">
              <div className="text-sm text-gray-400">Matches ({matches[it._id].length}):</div>
